...
class ProdukController extends Controller
{
    ...
    {
        $news = $this->newsService->getIndonesia();
        $articles = $news->articles;
        return view('frontend.berita')->with(['articles'=>$articles]);
    }

    public function cuaca()
    {
        $client = new Client();
        $response = $client->request('GET', 'https://api.openweathermap.org/data/2.5/weather?q=Jakarta,id&appid=9c37663ec51e2232de11ec3923c90bb6');
        //dd($response->getBody()->getContents()); buat dptin data masih bentuk jsonnya
        $data = json_decode($response->getBody(), true);
        //dd($data) di decode buat diubah jd struktur data
        return view('frontend.cuaca', ['data' => $data]);
    }
    ...
    public function addToCart(CartProductRequest $request, $slug)
    {
        ...
        $cart = Cart::where('user_id', \auth()->user()->id)->where('status', '=', 'dikeranjang')->get();
        if(!$cart->isEmpty())
        {
